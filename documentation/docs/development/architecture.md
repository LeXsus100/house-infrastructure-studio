...
Vite proxies `/api` to the Express server in development. After `npm run build`,
Express serves both `dist/` and `/api` from `127.0.0.1:4280`. Every application
listener stays on the loopback interface, and the runtime uses local
integrations.
...
`scripts/prepare-desktop.mjs` bundles `server/desktop.ts`, copies the current
Node.js 24 runtime as a target-triple Tauri sidecar, and obtains the matching
official Node.js license for the installer resources. Tauri marks that process
as the single desktop entry, while the client waits for `/api/health` before
loading project data. Tauri embeds the neutral Windows icons and WebView2
bootstrapper, then produces a current-user NSIS installer. Generated sidecars,
licenses, Rust targets, and installers are ignored.
...
- `src/editor/HouseViewport.tsx` renders schematic walls, rooms, devices, physically scaled route widths, measurement lines, the metric grid, and predictable orbit/pan/zoom controls. Rendering-only wall splitting assigns overlapping corner, crossing, and collinear volumes to one stable wall while persisted wall records stay separate.
...
- `src/components/RoutePortDialog.tsx` makes endpoint selection explicit in a compact two-column grid. Port occupancy is derived from route endpoint relationships; reassigning the previous route releases an occupied physical port. When the compatible endpoint list is empty, an embedded 3D device editor creates a precisely positioned instance port and resumes the pending route with its draft intact. Junction boxes and electrical panels expose the same editor continuously; `src/lib/devicePorts.ts` derives their minimum enclosure dimensions from each port's required termination area.
...
- `src/i18n/en.ts` and `src/i18n/it.ts` are the two local UI dictionaries. `src/lib/i18n.tsx` stores the EN/IT choice in browser-local storage, with English as the default. Translation stays inside these bundled dictionaries.
...
- `src/lib/elevation.ts` renders technical wall schemes directly to a local canvas.
- `src/lib/riser.ts` validates one-to-one, same-service cross-floor route links and groups linked floor records into physical sleeve lanes. `src/lib/diagnostics.ts` derives route, riser, panel/junction correspondence, and unconnected-device health lists from persisted relationships.
...
Wall and device meshes are simple low-poly primitives, and the project model keeps integer millimetres to avoid repeated floating-point drift. Visible curved-route geometry is memoized by project/view inputs, wall associations use indexed lookup, and each visible route fragment is submitted as one polyline for all sampled segments. The canvas renders on demand and switches to continuous frames while animated X-ray flow markers are active; compass updates bypass React state. The current first version is appropriate for normal residential models. Profiling will guide any future use of instanced meshes and spatial indexing.
