...
import { ArrowDown, ArrowUp, Crosshair, Eye, EyeOff, ImagePlus, Minus, Navigation, Plus, Trash2 } from 'lucide-react';
...
export function LevelManagerDialog(props: Props) {
  ...
  const upload = (file?: File) => {
    ...
    reader.onload = () => {
      ...
      image.onload = () => { setCalibration([]); setNorthPoints([]); props.onUpdate(selected.id, { blueprint: {
  ...
  return <div className="modal-backdrop level-manager-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && props.onClose()}>
    <div className="level-manager-dialog" role="dialog" aria-modal="true" aria-label="Manage levels">
      ...
      <div className="level-manager-body">
        ...
        {selected && <main>
          <section><h2>Level definition</h2><label className="field"><span>Name</span><input value={selected.name} onChange={(event) => props.onUpdate(selected.id, { name: event.target.value })} /></label><div className="field-grid"><label className="field"><span>Elevation</span><span className="input-with-suffix"><DraftNumberInput value={selected.elevationMm / 1000} step={.01} onCommit={(value) => props.onUpdate(selected.id, { elevationMm: Math.round(value * 1000) })} /><em>m</em></span></label><label className="field"><span>Ceiling height</span><span className="input-with-suffix"><DraftNumberInput min={.1} value={selected.ceilingHeightMm / 1000} step={.01} onCommit={(value) => props.onUpdate(selected.id, { ceilingHeightMm: Math.round(value * 1000) })} /><em>m</em></span></label></div></section>
          <section className="blueprint-settings"><div className="section-title-row"><div><h2>Blueprint underlay</h2><p>Zoom in, then place the two scale points precisely on a known line.</p></div><button onClick={() => fileInput.current?.click()}><ImagePlus size={15} /> {selected.blueprint ? 'Replace image' : 'Upload image'}</button></div><input ref={fileInput} hidden type="file" accept="image/*" onChange={(event) => upload(event.target.files?.[0])} />
            {!selected.blueprint ? <button className="blueprint-empty" onClick={() => fileInput.current?.click()}><ImagePlus size={24} /> Upload a floor-plan image</button> : <>
              <div className="blueprint-preview-controls"><div className="blueprint-mode-buttons"><button className={pointMode === 'calibration' ? 'active' : ''} onClick={() => setPointMode('calibration')}>Scale line</button><button className={pointMode === 'alignment' ? 'active' : ''} onClick={() => setPointMode('alignment')}><Crosshair size={14} /> Alignment point</button><button className={pointMode === 'north' ? 'active' : ''} onClick={() => setPointMode('north')}><Navigation size={14} /> North arrow</button></div><div className="blueprint-zoom"><button aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(.5, value - .25))}><Minus size={14} /></button><input aria-label="Blueprint preview zoom" type="range" min="0.5" max="4" step="0.25" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /><button aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(4, value + .25))}><Plus size={14} /></button><span>{Math.round(zoom * 100)}%</span></div></div>
              <div className="blueprint-preview-scroll"><div className="blueprint-calibration" style={{ width: `${zoom * 100}%`, aspectRatio: `${selected.blueprint.naturalWidth} / ${selected.blueprint.naturalHeight}` }} onClick={clickPreview}>
              ...
              <div className="blueprint-status-row"><span className={selected.blueprint.scaleLinePx && selected.blueprint.scaleLineLengthMm ? 'complete' : ''}>{selected.blueprint.scaleLinePx && selected.blueprint.scaleLineLengthMm ? `Scale saved · ${(selected.blueprint.scaleLineLengthMm / 1000).toFixed(2)} m` : selected.blueprint.scaleLinePx ? 'Scale line saved · enter its length' : 'Scale line not set'}</span><span className={selected.blueprint.alignmentPointPx ? 'complete' : ''}>{selected.blueprint.alignmentPointPx ? `Alignment point saved · ${Math.round(selected.blueprint.alignmentPointPx.x)}, ${Math.round(selected.blueprint.alignmentPointPx.z)} px` : 'Alignment point not set'}</span><span className={selected.blueprint.northArrowPx ? 'complete' : ''}>{selected.blueprint.northArrowPx ? `North saved · ${selected.blueprint.rotationDeg.toFixed(1)}°` : 'North arrow not set'}</span></div>
